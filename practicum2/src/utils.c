...
long send_all(int s, char* buf, long len) {
   ...
   */
  // FROM
  // https://stackoverflow.com/questions/33646539/should-send-and-recv-buffer-size-be-as-big-as-possible
...
long receive_all(int s, char* buf, long len) {
  ...
  while (total <
         len) { // while the read amount of data is smaller than teh chunk size
...
void generate_random_file(char* filePath, int size) {
  /**
   * Generate a text file with random alphanumeric characters  at the specified
   * file path.
   *
   * Args:
...
char* get_actual_fp(char* filePath, int remote) {
  /**
   * returns the actual path to the remote filesystem given a relative path from
   * its root. args: filePath (char*): relative path to file remote (int): 1 if
   * the file is on the remote filesystem, 0 otherwie(local)
   *
   */
...
char** split_str(char* str, char* delim) {
  ...
  char* strCopy =
      strdup(str); // copy the string to avoid modifying the original
...
void make_parent_dirs(char* filePath) {
  ...
  char* copyFilePath =
      strdup(filePath); // copy the file path to avoid modifying the original

  char* dir = dirname(copyFilePath);

  if (!directory_exists(dir)) {

    make_parent_dirs(dir); // recursively create parent directories until a
                           // direector that is already made is reached
    mkdir(dir, 0777);
  }
}
...
void remove_key_from_dict(dict* d, char* key) {
   ...
   *
   * DOES NOT FREE VALUES. Since we store pointers , it is the responsibility of
